...
export default function AiUploadModal({ isOpen, onClose, onSave }: AiUploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/documents/extract', {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) {
        console.warn('Failed to extract document data. Proceeding to blank form.');
        onSave({});
      } else {
            ...
            >
              {!file ? (
                <>
                  ...
                  <p className="text-sm text-gray-600 font-medium mb-1">Drag and drop your invoice or loan document here</p>
